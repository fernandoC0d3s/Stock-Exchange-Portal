...
unsigned int hash (const char *word)
{
    ...
    {
        hash = ((hash << 5) + hash) + c; /* hash * 33 + c *///(DJb2) uses this << as it denotes noation to shifting bits
    }
    return hash % N;
}    
...
bool load(const char *dictionary)
{
    ...
    while (fscanf(file,"%s\n", tempWord) != EOF)
    {
       node* t = malloc(sizeof(node)); // allocates memory of temp node
...
bool unload(void)
{
   for(int i = 0;  i < N; i++)
   {
       ...
       while (nodePtr != NULL) // i am in linked ist of hash possible value
       {
         ...
         free(temp); //we free it
       }
       table[i] = NULL;  //if is alrady lost GG
   }
    return true;
}
